...
# the glass alpha in the SVG, and Dialog gets an opaque element so dialogs stay solid.
#
# reduce_window_opacity must be nonzero or Kvantum skips translucency, but it dims every
# element, dialogs included, so it stays at 1% and the real alpha lives in the SVG.
{
  catppuccin-kvantum,
  lib,
  name,
