...
# usage: papirus-catppuccin.py SRC_THEME_DIR DST_THEME_DIR NAME PALETTE_JSON
# PALETTE_JSON holds the Catppuccin "mocha" and "latte" flavours as bare rrggbb strings.
palette = json.loads(pathlib.Path(sys.argv[4]).read_text())
mocha, latte = palette["mocha"], palette["latte"]
ACCENTS = ["red", "peach", "yellow", "green", "teal", "sky", "sapphire", "blue", "mauve", "pink"]
MOCHA = [mocha[a] for a in ACCENTS]
LATTE = [latte[a] for a in ACCENTS]
GLYPH = mocha["base"]
# Papirus gray lightness -> Catppuccin neutral, anchored so the paper sheet lands on "text". The top
# stop and the paper highlight are a little lighter than any Mocha colour.
NEUTRALS = [
    (0.00, mocha["crust"]),
    (0.25, mocha["base"]),
    (0.35, mocha["surface0"]),
...
PAPER = {"e4e4e4": mocha["text"], "fafafa": "e1e6f8"}
